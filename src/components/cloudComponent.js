...
const cloudShapeSvgs = [cloudSvgV1, cloudSvgV2, cloudSvgV3];
...
function getRandomCloudSvg(name, descript) {
  ...
  const taskName = escapeXml(name || 'Cloud Task');
  const taskNameShort =
    taskName.length > 12 ? `${taskName.substring(0, 10)}...` : taskName;
  const taskDesc = escapeXml(descript || 'A cloud representing a task.');
